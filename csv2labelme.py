...
for key, value in total_csv_annotations.items():
    ...
    for shape in value:
        label = shape[-1]
        s = {"label": label, "line_color": None,
             "fill_color": None, "shape_type": "rectangle"}
        points = [
            [shape[0], shape[1]],
            [shape[2], shape[3]]
        ]
    ...
    json.dump(labelme_format, open("%s/%s/" % (image_path,
              key.replace(".jpg", ".json")), "w"), ensure_ascii=False, indent=2)
