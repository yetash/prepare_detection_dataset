    ...
    for i in range(len(label)):
        ...
        if label[i] in cat2id.keys():
            ...
            for k,v in cat2id.items():
                if k.find(label[i])!= -1 or label[i].find(k) != -1:
                    label_id = v
                    break
        if label_id == -1:
            continue
        ...
        w = int(boxes[i][2])
        h = int(boxes[i][3])
        cv2.rectangle(im, (x,y), (w, h),colorlist[label_id], 1)
        cv2.putText(im, label[i], (x,y+15), cv2.FONT_HERSHEY_COMPLEX_SMALL,1,colorlist[label_id])
    if showim:
        cv2.imshow("", im)
        cv2.waitKey()
